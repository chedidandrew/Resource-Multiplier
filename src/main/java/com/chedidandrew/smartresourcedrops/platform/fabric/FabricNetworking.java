...
final class FabricNetworking {
    ...
    static void register() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(ConfigPatchPayload.TYPE,
                (server, player, handler, buffer, responseSender) -> {
                    final ConfigPatchPayload payload = ConfigPatchPayload.read(buffer);
                    server.execute(() -> SmartDropsNetworking.handlePatch(payload, player));
                });
