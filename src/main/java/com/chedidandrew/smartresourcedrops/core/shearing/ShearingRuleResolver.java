    ...
    static ShearingRuleTrace trace(
        ...
        if (classification == ShearingClassification.UNKNOWN) {
            return result(
                    ...
                    standardTagged
                            ? "Minecraft 1.20.5-1.20.6 supports final-output multiplication for sheep only; other standard-resource tag members remain vanilla 1x"
                            : "unknown shearables are fixed at vanilla 1x on Minecraft 1.20.5-1.20.6");
        }
        if (!config.enabled) {
            return result(
                    entityId,
