...
4. Launch Gradle with Java 21, then run the Fabric suite with `./gradlew --no-daemon clean test runGameTest build` and `./gradlew --no-daemon runClientSmoke` (use `gradlew.bat` on Windows). Gradle uses the `java_version` toolchain declared in `gradle.properties`—Java 17 on this branch—for compilation and Minecraft test launches.
5. Run the NeoForge suite with `./gradlew -p neoforge --no-daemon clean test build runGameTestServer`, followed by the client, physical multiplayer, optional-installation, and hostile-wire gates listed in `docs/TESTING.md`.
