...
4. Run the Fabric suite with `./gradlew --no-daemon clean test runGameTest build` and `./gradlew --no-daemon runClientSmoke` using the Java toolchain declared by `java_version` in `gradle.properties` (Java 17 on this branch; use `gradlew.bat` on Windows).
