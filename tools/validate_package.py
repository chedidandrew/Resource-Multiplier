...
form_markers = {
    ".github/ISSUE_TEMPLATE/bug_report.yml": (
        ...
        "placeholder: 1.3.2+mc1.21.2-1.21.3",
        'placeholder: "21"',
        "NeoForge",
        "Java version",
...
expected_properties = {
    "mod_version": "1.3.2+mc1.21.2-1.21.3",
...
for marker in (
    ...
    "actions/workflows/build.yml/badge.svg?branch=backport%2F1.21.2-1.21.3",
    "Minecraft-1.21.2--1.21.3",
    "Loaders-Fabric%20%7C%20NeoForge",
    "Java-21",
    "License-MIT",
    "Status-1.3.2%2Bmc1.21.2--1.21.3-Stable",
    "> [!IMPORTANT]",
...
if isinstance(mixin, dict):
    ...
    if isinstance(listed, list):
        if listed != expected_production_mixins:
            fail(
                "Production mixin configuration must declare the exact audited 1.21.2-1.21.3 set: "
...
for game_test_contract in (
    ...
    "Packaged NeoForge server candidate Smart Resource Multiplier 1.3.2+mc1.21.2-1.21.3",
...
if (
    ...
    or "dist/smart-resource-multiplier-1.3.2+mc1.21.2-1.21.3.jar" not in release_workflow
    or "dist/smart-resource-multiplier-neoforge-1.3.2+mc1.21.2-1.21.3.jar" not in release_workflow
):
...
for required_release_gate in (
    ...
    "refs/tags/v1.3.2+mc1.21.2-1.21.3",
    'test "$release_ready" = "true"',
    'test "$tag_commit" = "$branch_commit"',
    "refs/remotes/origin/backport/1.21.2-1.21.3",
    "fetch-depth: 0",
    "make_latest: false",
):
