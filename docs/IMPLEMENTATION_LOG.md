...
The README was rebuilt as a focused public landing page. It leads with the production icon and concise project identity, uses current real-client screenshots rather than fabricated or placeholder media, distinguishes automated evidence from still-open release gates, and routes installation, configuration, compatibility, testing, and build details into the maintained documentation. The exhaustive command tree moved to [`docs/COMMANDS.md`](COMMANDS.md), so operators retain a complete reference without overwhelming the landing page.

Workflow ownership is now unambiguous: the normal build workflow handles branch pushes, pull requests, and manual runs but no longer handles `v*` tags; the release workflow is the sole tag handler and still requires its explicit latch and publication checks. No tag, GitHub Release, or other publication was created during this clarity pass.

The public inspection/status copy now labels entity attribution as hypothetical and describes the shearing bound as 256 source entries or materialized legal stacks. The command reference also records the master/source disable gates and whole-action over-budget fallback. These are presentation corrections to existing behavior, not gameplay or limit changes.

Automated validation establishes candidate evidence but does not close the manual hands-on gameplay, separately installed multiplayer, live datapack-reload, restart/migration, or named third-party compatibility gates. Representative biome/passive, hostile, boss, inventory-bearing mob, custom-shearable, tool, and automated-machine integrations remain required before promoting the candidate to final `1.2.0` or enabling publication.
...
Commands, read-only inspection, live validation, focused GUI screens, package manifests, and documentation were extended. The serial Java 25 chain passed 158 JUnit tests, all 66 dedicated GameTests (44 shearing-focused), the real client GameTest, the clean Loom build, six static/core validators, and standalone server/client startup smokes. At that historical checkpoint, version metadata still remained 1.1.0 and the entry made no release claim; the later `1.2.0-rc.1` identity and current open gates are recorded at the top of this log.
