...
## Current 1.21.6-1.21.8 backport status

The `backport/1.21.6-1.21.8` branch provides one shared Fabric file and two version-scoped NeoForge files with the same schema 3 configuration, commands, GUI, and gameplay policy. The complete loader/version matrix and manual UI checks pass, and the guarded release commit is authorized. It deliberately makes no Minecraft world-downgrade or cross-loader placed-block-data migration promise.

## Historical Minecraft 26.2 / 1.3.0 status

This section records the newer Minecraft 26.2 line from which this backport was made. It is historical context on this branch, not a compatibility or migration claim for the 1.21.6-1.21.8 artifacts.
