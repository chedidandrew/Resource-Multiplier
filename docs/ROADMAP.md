...
## Current 1.21.2–1.21.3 backport status

The release-ready `backport/1.21.2-1.21.3` branch targets Fabric and NeoForge with the same schema 3 configuration, commands, GUI, and gameplay policy as the current dual-loader implementation. Loader-native server, client, packaged-JAR, artifact-validation, and manual acceptance gates pass. Minecraft world downgrades and cross-loader placed-block-data migration remain unsupported.
