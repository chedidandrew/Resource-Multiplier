...
Version `1.3.0` is the stable Minecraft 26.2 release for Fabric and NeoForge. The two JARs share gameplay, schema 3 configuration, commands, GUI, safety budgets, and the `smart_resource_drops` compatibility identity while using loader-specific lifecycle, networking, and placed-block-storage adapters.

## Identity and compatibility

- [x] Fabric and NeoForge expose the public name **Smart Resource Multiplier** and version `1.3.0`.
...
- [x] All 66 Fabric dedicated-server GameTests pass.
- [x] The real Fabric client GUI/authority GameTest passes with the updated block-XP wording.
- [x] The Java 25 Fabric Loom build completes and produces a test-free, dependency-clean playable JAR.

## NeoForge gates

- [x] Clean Java 25 build, 164 JUnit tests, and all 65 NeoForge dedicated-server GameTests pass.
...
- [x] The NeoForge JAR validator rejects Fabric crossover, test/probe fixtures, nested dependencies, metadata drift, missing mixins, incorrect icon bytes, and non-Java-25 bytecode.

## Packaging and distribution

- [x] Fabric and NeoForge `mod_version` values match exactly at `1.3.0` and `release_ready=true` is set only after the parity gates pass.
- [x] Deterministic packaging requires both freshly rebuilt, separately validated JARs.
- [x] Fabric filename: `smart-resource-multiplier-1.3.0.jar`.
- [x] NeoForge filename: `smart-resource-multiplier-neoforge-1.3.0.jar`.
...
- [x] Stable source is pushed to `main`. A GitHub tag/Release remains a separate optional action and is not required for the CurseForge distribution requested here.
