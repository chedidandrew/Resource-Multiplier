# NeoForge 1.21.2–1.21.3 backport

## Baseline and layout

The NeoForge build compiles against Minecraft `1.21.2` and NeoForge `21.2.1-beta`, declares compatibility through Minecraft 1.21.3, and uses ModDevGradle `2.0.146` with Java 21. It consumes the canonical common and client source trees while excluding Fabric adapters, then adds NeoForge entrypoints, networking, placement storage, mixins, metadata, and target-native tests under `neoforge/`.
...
- NeoForge-specific placement and shearing mixins use exact target method descriptors with required injection counts.
...
Standard living-entity shearing uses the safe final-output boundary available on these targets. Special transformations and unsupported shearing paths stay `1x`.
...
The established filenames are `smart-resource-multiplier-1.3.2+mc1.21.2-1.21.3.jar` for Fabric and `smart-resource-multiplier-neoforge-1.3.2+mc1.21.2-1.21.3.jar` for NeoForge. Install only the file matching the chosen loader.
