# NeoForge 1.21.4 backport

## Baseline and layout

The NeoForge build targets Minecraft `1.21.4`, NeoForge `21.4.157`, ModDevGradle `2.0.146`, and Java 21. It consumes the canonical common and client source trees while excluding Fabric adapters, then adds NeoForge entrypoints, networking, placement storage, mixins, metadata, and target-native tests under `neoforge/`.
...
- NeoForge-specific placement and shearing mixins use exact 1.21.4 method descriptors with required injection counts.
...
The established filenames are `smart-resource-multiplier-1.3.2+mc1.21.4.jar` for Fabric and `smart-resource-multiplier-neoforge-1.3.2+mc1.21.4.jar` for NeoForge. Install only the file matching the chosen loader.
