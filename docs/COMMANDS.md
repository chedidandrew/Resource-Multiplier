...
This reference applies to **Resource Multiplier 1.2.0-rc.1**. The public name changed, but the command entry points remain legacy-compatible: server commands use `/smartdrops`, and the client configuration screen uses `/smartdropsgui`.
...
There are no `/smartdrops admin entity ...` mutation branches in 1.2.0-rc.1. Entity-death settings are edited through the Entity Drops configuration screens or `config/smart_resource_drops.json`; `/smartdrops inspect entity` and `/smartdrops validate` remain the command-line diagnostics for that domain.
