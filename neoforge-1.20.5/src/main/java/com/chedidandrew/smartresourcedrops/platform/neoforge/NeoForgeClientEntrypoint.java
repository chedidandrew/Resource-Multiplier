...
/** Physical-client bootstrap invoked through DistExecutor on NeoForge 20.5. */
public final class NeoForgeClientEntrypoint {
    ...
    public NeoForgeClientEntrypoint(final IEventBus modBus, final ModContainer container) {
        ...
        launchClientCategorySmokeIfRequested();
    }

    private static void launchClientCategorySmokeIfRequested() {
        if (!Boolean.getBoolean("smart_resource_drops.clientCategoryTest")) {
            return;
        }
        try {
            Class.forName("com.chedidandrew.smartresourcedrops.client.NeoForgeClientCategorySmokeTest")
                    .getConstructor()
                    .newInstance();
        } catch (ReflectiveOperationException failure) {
            throw new IllegalStateException("Could not start the NeoForge 1.20.5 client smoke", failure);
        }
    }
