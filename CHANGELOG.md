...
## 1.3.2+mc1.21.2-1.21.3 - 2026-09-04

### Added

- Added release-ready Fabric and NeoForge support for Minecraft 1.21.2 and 1.21.3 from one JAR per loader.
...
- Passed loader-native builds, unit suites, all 64 GameTests, exact packaged runtime probes, GUI checks, and manual testing across both Minecraft versions and loaders.
...
- Applied a minimal, presentation-only configuration clarity pass. On the root screen only, **Categories** and **Filters** now read **Block Categories** and **Block Filters** while the child-screen titles remain unchanged. Dimensions, Advanced, and Entity Drops gain concise scope tooltips; the two root XP tooltips now identify eligible block breaks; and the eight Advanced boolean rows explain their behavior instead of repeating only their label and ON/OFF state. Existing detailed tooltips remain unchanged, and this pass changes no gameplay, configuration semantics, schema, defaults, persistence, networking, commands, namespaces, or compatibility identifiers. Local validation and exact clean-checkout GitHub Actions run `33354997205` pass for production commit `cb6d17d9a26283ddc98c6e3f7d8fa46ebb341380`.
...
- The configuration clarity chain passes all six repository validators with 118 production Java sources and 257 source-package entries, all 90 core assertions, 158 JUnit tests in 24 suites, all 66 dedicated GameTests, the 40-second Mod Menu-enabled real client GameTest, the 31-second cache-free Java 25 build, and the 42-second standalone server smoke. Runtime GUI coverage verifies exact tooltip copy, root-only labels, unchanged child titles and preset help, shared multiplier-control help, 170-pixel wrapping, on-screen positioning from 320x180 through 1280x720, and read-only non-mutation. Fresh packaging produces a 601,761-byte/311-entry JAR with SHA-256 `27F73D54F8CD6EFE17F68DFB3C76D11B67BEAB3AB6DF4725F0D398E4D0E5F9C7`.
- Exact clean-checkout GitHub Actions run `33354997205` passes for production commit `cb6d17d9a26283ddc98c6e3f7d8fa46ebb341380`, including package policy checks, Java/dedicated tests and build, client GUI/authority tests, and playable-JAR upload.
- At the earlier public-presentation checkpoint, the local chain passed package/README metadata validation with 118 production Java sources, deterministic release validation with 252 Git-tracked source entries, Mod Menu/edge-case/polish audits, 90 core assertions, 158 JUnit tests in 24 suites, all 66 dedicated GameTests, the 39-second real client GameTest with Mod Menu present, and the 29-second cache-free clean Loom build. Its standalone server loaded without Mod Menu, reached `Done (0.247s)`, returned valid general/shearing status and validation output, and stopped cleanly in 48 seconds.
- At that earlier checkpoint, fresh-empty-directory packaging produced the versioned source ZIP, playable JAR, checksum manifest, and release bundle. Its inspected 601,044-byte/311-entry JAR had SHA-256 `5CA797D6BC4BBAB6F223361D9A99A118850F40B0273426D3B1ADF8AAB5CDCB31` and retained exact MIT/contact/icon/entrypoint/mixin/dependency metadata, every production top-level class, the `smart_resource_drops` data namespace, and zero nested JAR or GameTest entries.
