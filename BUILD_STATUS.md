...
Structured configuration tooltips now use Minecraft's standard 170-pixel splitter before rendering. This prevents screen-wide tooltip lines and consumes embedded line breaks instead of exposing `LF` control glyphs. Entity-category hover text was shortened to supplemental details, and the real client GameTest verifies every category tooltip with the mapped runtime font: all lines remain within 170 pixels and no LF or CR code point survives rendering.
...
- The latest `runClientGameTest --no-build-cache --rerun-tasks --warning-mode all` completed with `BUILD SUCCESSFUL` in 42 seconds after exercising title-screen, integrated-server, dedicated-server editor/authority, configuration-title, reset-confirmation, and mapped tooltip wrapping/control-character paths.
- The latest final `clean build --no-build-cache --rerun-tasks --warning-mode all` completed with `BUILD SUCCESSFUL` in 35 seconds and reran the 158 JUnit tests and all 66 dedicated GameTests.
...
The freshly rebuilt candidate artifact is `build/libs/resource-multiplier-1.1.0.jar`: 601,037 bytes, 311 ZIP entries, SHA-256 `3455FA41295A88392879B63F794020389A03DBE7A2A8777455DB73273CB4A76D`, embedded mod ID `smart_resource_drops`, public name `Resource Multiplier`, version 1.1.0, Java class major 69, exact root-matching `LICENSE_resource-multiplier`, canonical homepage/source/Issues metadata, the stable `data/smart_resource_drops` namespace, and zero GameTest classes or nested JARs. The legacy-name same-version JAR under `dist/` is an older historical 1.1.0 artifact with no shearing classes and must not be used as this candidate.
