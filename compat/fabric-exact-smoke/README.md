...
1.21.3 with Fabric Loader 0.19.5 and Fabric API 0.114.1+1.21.3.
...
.\compat\fabric-exact-smoke\Run-FabricExactSmoke.ps1 `
  -CandidateJar ".\build\libs\smart-resource-multiplier-1.3.2+mc1.21.2-1.21.3.jar" `
  -AcceptEula
```
...
- creates a unique directory under `runtime/fabric-1.21.3/`;
...
and cleanly stopped on a Fabric 1.21.3 dedicated server. It does not exercise the
client configuration GUI and is not, by itself, proof of gameplay equivalence.
Those require the existing GameTests plus a client smoke test.
